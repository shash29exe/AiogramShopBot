...
async def edit_quantity_items(callback: CallbackQuery, bot: Bot):
    """
    ...
    if not product or not user_cart:
        await callback.answer('Продукт или корзина не найдены')
    ...
    if action == '+':
        ...
        await callback.answer('Количество товаров не может быть меньше 1')
        return
    ...
    caption = text_for_caption(name=product.product_name, description=product.description, price=total_price)
    try:
